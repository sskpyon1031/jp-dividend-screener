...
yfinance で 株価 / 時価総額 / 予想配当利回り を取得
        │  加えて日足1年ぶんをまとめて取得し 25日/75日移動平均・GC を算出
...
サイト側でも利回り・時価総額・業種・並び順に加え、テクニカルで絞り込めます(条件は端末に保存):
「25日線が上向き」「上向きに転じて10日以内(初動)」「株価が25日線より上」
「ゴールデンクロスが新しい(20日以内)」「ゴールデンクロス間近」。

移動平均のパラメータは `fetch.py` の定数で調整できます:
...
| `days_since_golden_cross` | GC成立から何営業日か(未クロス/1年以上前は `null`) |
